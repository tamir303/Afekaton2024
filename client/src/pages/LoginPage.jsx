...
const LoginPage = ({setUser}) => {
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const userBoundary = { email: email, platform: "Afektive", role: role, userDetails: {"password": password} }
            console.log(userBoundary)
            const response = await axios.post(`${process.env.REACT_APP_BACKEND_DEV_URL}/entry/login`, userBoundary);
            if (response.status === 200) {
                setUser(userBoundary);
                console.log(
